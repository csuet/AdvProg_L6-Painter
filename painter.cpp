...
void Painter::setColor(SDL_Color color)
{
    ...
    this->color = color;
    SDL_SetRendererDrawColor(renderer, color.r, color.g, color.b);
}
...
void Painter::jumpForward(int numPixel)
{
    ...
    x += static_cast<int>(numPixel * cos(angle / 180) * M_PI);
    y -= static_cast<int>(numPixel * sin(angle / 180) * M_PI);
}
...
void Painter::turnLeft(double degree)
{
    ...
    double tmp_angle = getAngle() + degree;
    while (tmp_angle >= 360)
    {
        tmp_angle -= 360;
    }
